...
| Mode | What happens |
|------|-------------|
| **full** (default) | Writes `~/.cursor/rules/presentation-generator.mdc` (Cursor rule) **and** registers the MCP server in `~/.cursor/mcp.json`, enabling all **11** MCP tools: `render_deck`, `export_deck`, `audit_deck`, `judge_deck`, `list_themes`, `apply_theme`, `generate_deck_prompt`, `preview_themes`, `import_pptx`, `import_markdown`, `import_brand_theme`. |
