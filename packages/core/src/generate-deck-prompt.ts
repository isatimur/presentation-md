/**
 * One-shot craft prompt builder — shared by MCP `generate_deck_prompt` and CLI `--generate-prompt`.
 */
import { readFile } from "node:fs/promises";
import { join } from "node:path";
import { getCorePackageRoot, loadLayoutRecipesMarkdown, loadThemeShortlists } from "./theme-discovery.js";
import { loadTheme } from "./theme-loader.js";
import type { DensityMode } from "./remorph-density.js";
import type { Typography } from "./theme-types.js";

export interface BuildGenerateDeckPromptOptions {
  theme?: string;
  intent?: string;
  density?: DensityMode;
  /** Theme search roots — defaults to bundled core themes. */
  themesDir?: string;
  fallbackThemesDirs?: string[];
}

export interface GenerateDeckPromptResult {
  theme: string;
  intent: string;
  density: DensityMode;
  ...
  themes_reference?: string;
  stunning_25_reference?: string;
...
function buildCraftMandate(density: DensityMode): string {
  const densityLine =
    density === "reading"
  ...
  return [
    ...
    "- If density drifts after drafting, call audit_deck with remorph_density speaker|reading (CLI --remorph-density / Studio Speaker·Reading density) before rewriting copy — structural remorph is non-LLM.",
    "- Ban Inter-only / purple-on-white / cream-terracotta defaults unless the chosen theme owns them (see anti_slop_reference).",
    "- When theme is unsure: use theme_shortlists_reference (or list_themes browse/include_browse_filters/include_shortlists/mood/query) — offer the same mood chips as the site/Studio gallery, prefer suggested_preview (safe + bold + wildcard), or pick a 3-theme shortlist, call preview_themes (≥2 themes auto-defaults to mode=\"layouts\"; inline PNGs on by default — title + bento + comparison; pass mode=\"title\" only for a cover skim), then lock meta.theme. Studio parity: pick-3 / Generate / Example featured all show densified Title/Bento/Compare shot strips (shared iframe · scroll-crop). Default shortlist for vague briefs: core-defaults (prefer popular:true / browse:\"popular\").",
...
export async function buildGenerateDeckPrompt(
  opts: BuildGenerateDeckPromptOptions = {}
): Promise<GenerateDeckPromptResult> {
  const themeName = opts.theme?.trim() || "default-tech";
  const intent = opts.intent ?? "";
  const density: DensityMode = opts.density === "reading" ? "reading" : "speaker";

  const coreRoot = getCorePackageRoot();
  const themesDir = opts.themesDir ?? join(coreRoot, "themes");
  const fallbackThemesDirs = opts.fallbackThemesDirs;
  ...
  return {
    ...
    intent,
    density,
    craft_mandate: buildCraftMandate(density),
    skill,
