...
/** Hard 0-blur card offset shadows from HTML neobrutalist peers (raw-grid / hard-bento / …). */
function cardHardOffset(ctx: ExportContext): { color: string; size: number } | null {
  if (ctx.themeName === "raw-grid") return { color: "0A0A0A", size: 0.045 };
  if (ctx.themeName === "genz-bento") return { color: ctx.colors.text, size: 0.045 };
  if (ctx.themeName === "creative-mode") return { color: ctx.colors.text, size: 0.045 };
  if (ctx.themeName === "block-frame") return { color: "000000", size: 0.065 };
  // peoples-platform-poster cards: 6px red offset
  if (ctx.themeName === "peoples-platform") return { color: "E83A2A", size: 0.065 };
  // capsule-pills cards: soft ink offset stand-in
  if (ctx.themeName === "capsule") return { color: "1A1A1A", size: 0.06 };
  // scatterbrain sticky notes: quiet cork offset
  if (ctx.themeName === "scatterbrain") return { color: "2D2A26", size: 0.04 };
  // daisy-days-pastel cards: 4px charcoal offset
  if (ctx.themeName === "daisy-days") return { color: "2D2D2D", size: 0.05 };
  // sakura-chroma-cassette cards: accent-tinted hard offset (HTML 10px)
  if (ctx.themeName === "sakura-chroma") return { color: "E5392A", size: 0.08 };
  // bauhaus-blocks cards: quiet ink offset stand-in
  if (ctx.themeName === "bauhaus") return { color: "0D0D0D", size: 0.04 };
  return null;
}
...
function cardStroke(
  ...
  // daisy-days-pastel cards: 3px charcoal + hard offset
  if (ctx.themeName === "daisy-days") {
    return { color: "2D2D2D", width: opts.hero || opts.highlighted ? 3 : 2.75 };
  }
  ...
  // sakura-chroma-cassette cards: 2px ink + accent hard offset
  ...
  // hard-bento (genz-bento) cards: 2.5px ink + hard offset
  if (ctx.themeName === "genz-bento") {
    return { color: ctx.colors.text, width: opts.hero || opts.highlighted ? 2.9 : 2.65 };
  }
  // acid-block (brutalist-acid) cards: 2px accent, square
  if (ctx.themeName === "brutalist-acid") {
    return { color: ctx.colors.accent, width: opts.hero || opts.highlighted ? 2.25 : 2 };
  }
  // bauhaus-blocks cards: 2px ink + quiet offset, square
  if (ctx.themeName === "bauhaus") {
    return { color: ctx.colors.text, width: opts.hero || opts.highlighted ? 2.5 : 2.25 };
  }
  // neo-grid-panels / bold-poster-ink / tri-tone-blocks cards: hard ink, square
  if (
  ...
  // raw-grid-brutal cards: 3px ink + hard offset (via cardHardOffset), square
  if (ctx.themeName === "raw-grid") {
    return { color: "0A0A0A", width: opts.hero || opts.highlighted ? 3.5 : 3.25 };
  }
...
function drawCardFace(
  ...
  const offset = cardHardOffset(ctx);
  if (offset && !(stroke.topRule || stroke.leftRule)) {
    // Paint hard offset plate first (HTML box-shadow Nx N 0 ink).
    slide.addShape(ctx.shapeRoundRect, {
      x: x + offset.size,
      y: y + offset.size,
      w,
      h,
      fill: { color: offset.color },
      line: { color: offset.color, width: 0 },
      rectRadius: radius,
    });
  }
...
function paintSlideChrome(slide: PSlide, ctx: ExportContext, data: Slide): void {
  ...
  if (theme === "swiss-typographic") {
    // Denser modular grid (~64px) + thicker signal-red left rail (swiss-grid).
    for (let i = 1; i < 20; i++) {
      slide.addShape(ctx.shapeRoundRect, {
        x: (ctx.width / 20) * i,
        y: 0,
        w: 0.011,
        h: ctx.height,
        fill: { color: ctx.colors.border, transparency: 48 },
    ...
    // Quiet outer hairline (HTML border).
    slide.addShape(ctx.shapeRoundRect, {
      x: 0.1,
      y: 0.08,
      w: ctx.width - 0.18,
  ...
  if (theme === "bauhaus") {
    // Hard 2.5pt frame + primary geometry. Heroes get larger red/blue/yellow
    // blocks; body slides keep quieter corner primaries (HTML hides ::before/::after
    // on content, but Primary still wants geometry on every slide).
    // Cards get quiet ink offsets via cardHardOffset.
    slide.addShape(ctx.shapeRoundRect, {
      x: 0.08,
      y: 0.08,
      w: ctx.width - 0.16,
    ...
    const sq = isHero ? 1.1 : 0.55;
    const circ = isHero ? 1.05 : 0.52;
    const yel = isHero ? 0.55 : 0.32;
    const sqX = isHero ? ctx.width - 2.35 : ctx.width - 1.35;
    const sqY = isHero ? 0.65 : 0.45;
    const circX = isHero ? ctx.width - 2.95 : ctx.width - 1.85;
    const circY = isHero ? 1.35 : 0.85;
    slide.addShape(ctx.shapeRoundRect, {
      x: sqX,
      y: sqY,
      w: sq,
    ...
    slide.addShape(ctx.shapeOval, {
      ...
      w: circ,
      h: circ,
      fill: { color: ctx.colors.accent2, transparency: isHero ? 8 : 18 },
      line: { color: ctx.colors.accent2, width: 0 },
    });
    // Tertiary yellow block (HTML Primary gallery / stat bar accent).
    slide.addShape(ctx.shapeRoundRect, {
      x: isHero ? ctx.width - 1.55 : ctx.width - 0.95,
      y: isHero ? 1.95 : 1.15,
      w: yel,
      h: yel,
      fill: { color: "F4D35E" },
      line: { color: "F4D35E", width: 0 },
      rectRadius: 0,
    });
    // Quiet bottom-left ink stub for modernist grounding.
    slide.addShape(ctx.shapeRoundRect, {
      x: 0.45,
      y: ctx.height - 0.55,
      w: isHero ? 0.85 : 0.55,
  ...
  if (theme === "genz-bento") {
    ...
    // Cards get 4px hard offsets via cardHardOffset.
    const shadow = 0.12;
    slide.addShape(ctx.shapeRoundRect, {
      x: ctx.width - shadow,
      y: shadow,
    ...
    // Secondary accent chip (HTML hard-bento energy).
    slide.addShape(ctx.shapeRoundRect, {
      x: ctx.width - 1.35 - shadow,
  ...
  if (theme === "neo-grid-bold") {
    // Denser modular grid (~80px) + lemon corner panel + hard 3pt border +
    // 8px accent offset shadow (neo-grid-panels).
    const shadow = 0.1;
    slide.addShape(ctx.shapeRoundRect, {
      x: ctx.width - shadow,
      y: shadow,
    ...
    // Lemon corner panel (HTML ::after 96px) — always-on for PPTX densify.
    slide.addShape(ctx.shapeRoundRect, {
      x: ctx.width - 1.7 - shadow,
      y: 0.45,
      w: 1.15,
      h: 1.15,
      fill: { color: ctx.colors.accent },
    ...
    // Ink L registration marks (grid craft).
    slide.addShape(ctx.shapeRoundRect, {
      x: 0.22,
      y: 0.22,
      w: 0.48,
  ...
  if (theme === "bold-poster") {
    // Hard frame + accent-tinted offset shadow every slide; thicker ink top bar;
    // poster block on heroes (bold-poster-ink HTML uses accent@35% shadow).
    const shadow = 0.11;
    slide.addShape(ctx.shapeRoundRect, {
      x: ctx.width - shadow,
      y: shadow,
    ...
    if (isHero) {
      ...
      // Quiet poster stub on body slides so chrome doesn't go flat.
      slide.addShape(ctx.shapeRoundRect, {
        x: ctx.width - 1.35 - shadow,
        y: ctx.height - 1.15 - shadow,
        w: 0.85,
        h: 0.85,
        fill: { color: ctx.colors.bg2, transparency: 35 },
  ...
  if (theme === "capsule") {
    ...
    // coral pill + lime circles denser (always-on soft blot; loud pills hero-gated).
    // Cards get soft ink offsets via cardHardOffset.
    const shadowX = 0.1;
    const shadowY = 0.12;
    slide.addShape(ctx.shapeRoundRect, {
      x: ctx.width - shadowX,
    ...
    slide.addShape(ctx.shapeRoundRect, {
      x: 0.16,
      y: 0.16,
      w: ctx.width - 0.32 - shadowX,
      h: ctx.height - 0.32 - shadowY,
      fill: { color: ctx.colors.bg, transparency: 100 },
      line: { color: ctx.colors.border, width: 2.25 },
      rectRadius: 0.29,
    });
    // Quiet lavender / sky / lime radial blots (HTML --slide-bg candy — always-on).
    slide.addShape(ctx.shapeOval, {
      x: 0.35,
      y: ctx.height - 1.85 - shadowY,
      w: 0.78,
      h: 0.78,
      fill: { color: "C5B5E0", transparency: 38 },
      line: { color: "C5B5E0", width: 0 },
    });
    slide.addShape(ctx.shapeOval, {
      ...
      w: 0.62,
      h: 0.62,
      fill: { color: "8BB4F7", transparency: 34 },
      line: { color: "8BB4F7", width: 0 },
    });
    slide.addShape(ctx.shapeOval, {
      x: ctx.width * 0.78 - shadowX,
      y: ctx.height * 0.78 - shadowY,
      w: 0.42,
      h: 0.42,
      fill: { color: "C4D94E", transparency: 42 },
      line: { color: "C4D94E", width: 0 },
    });
    if (isHero) {
      slide.addShape(ctx.shapeRoundRect, {
  ...
  if (theme === "brutalist-mono") {
    // Denser mono grid (~24px) + hard 3pt outer frame + 8px ink offset shadow
    // (brutalist-grid).
    const shadow = 0.1;
    slide.addShape(ctx.shapeRoundRect, {
      x: ctx.width - shadow,
  ...
  if (theme === "scatterbrain") {
    // Cork wash + soft ink offset + denser rules + sticky notes + pins;
    // always-on tape strip densify (scatterbrain-cork). Cards get cork offsets.
    const shadow = 0.1;
    slide.addShape(ctx.shapeRoundRect, {
      x: ctx.width - shadow,
      y: shadow,
    ...
    // Soft cork yellow + blue washes (HTML radials).
    slide.addShape(ctx.shapeOval, {
      x: -0.8,
      y: -0.6,
      w: ctx.width * 0.55,
      h: ctx.height * 0.55,
      fill: { color: "FFE066", transparency: 84 },
    ...
    slide.addShape(ctx.shapeOval, {
      ...
      w: ctx.width * 0.5,
      h: ctx.height * 0.55,
      fill: { color: "74C0FC", transparency: 90 },
      line: { color: "74C0FC", width: 0 },
    });
    slide.addShape(ctx.shapeRoundRect, {
      x: 0.1,
      y: 0.1,
      ...
      h: ctx.height - 0.1 - shadow,
      fill: { color: ctx.colors.bg, transparency: 100 },
      line: { color: "2D2A26", width: 1.25 },
      rectRadius: 0.08,
    });
    // Quiet cork rules (HTML repeating ~40px lines — denser).
    for (let i = 1; i < 14; i++) {
      slide.addShape(ctx.shapeRoundRect, {
        x: 0.12,
        y: (ctx.height / 14) * i,
        w: ctx.width - 0.12 - shadow,
        h: 0.01,
        fill: { color: "2D2A26", transparency: 90 },
        line: { color: "2D2A26", width: 0 },
        rectRadius: 0,
      });
    }
    slide.addShape(ctx.shapeRoundRect, {
      ...
      h: 1.35,
      fill: { color: "FFE066" },
      line: { color: ctx.colors.text, width: 0.9 },
      rectRadius: 0.02,
    });
    slide.addShape(ctx.shapeRoundRect, {
    ...
    // Always-on tape strip densify (HTML ::after is hero-gated).
    slide.addShape(ctx.shapeRoundRect, {
      x: 0.55,
      y: ctx.height - 1.05 - shadow,
      w: 1.05,
      h: 0.28,
  ...
  if (theme === "coral") {
    // Hatch frame + coral left wash + bg2 diagonal split (coral-hatch).
    // Denser hatch + 3pt frame matching HTML border.
    slide.addShape(ctx.shapeRoundRect, {
      x: ctx.width * 0.62,
      y: 0,
      w: ctx.width * 0.38,
    ...
    // Diagonal hatch stand-ins (thin ink stripes — denser).
    for (let i = 0; i < 22; i++) {
      slide.addShape(ctx.shapeRoundRect, {
        x: -0.5 + i * 0.72,
        y: 0,
        w: 0.035,
        h: ctx.height,
        fill: { color: ctx.colors.text, transparency: 86 },
        line: { color: ctx.colors.text, width: 0 },
        rectRadius: 0,
      });
    }
  ...
  if (theme === "peoples-platform") {
    // Cobalt top bar + amber footer stub + hard 6pt frame + denser speckled wash
    // (peoples-platform-poster). Cards get 4px hard ink + red offsets.
    for (let i = 0; i < 48; i++) {
      const col = i % 8;
      ...
      slide.addShape(ctx.shapeOval, {
        ...
        w: 0.055,
        h: 0.055,
        fill: { color: ctx.colors.text, transparency: 90 },
        line: { color: ctx.colors.text, width: 0 },
      });
    }
    slide.addShape(ctx.shapeRoundRect, {
      ...
      h: ctx.height - 0.16,
      fill: { color: ctx.colors.bg, transparency: 100 },
      line: { color: ctx.colors.text, width: 4 },
      rectRadius: 0,
    });
    slide.addShape(ctx.shapeRoundRect, {
      x: 0,
  ...
  if (theme === "raw-grid") {
    // Blush/sage bands + cross rules + hard 3pt frame + 6px ink offset shadow
    // (raw-grid-brutal). Cards get 4px hard offsets via cardHardOffset.
    const shadow = 0.09;
    slide.addShape(ctx.shapeRoundRect, {
      x: ctx.width - shadow,
      y: shadow,
    ...
    // Ink corner blot + sage secondary stub (brutal grid craft).
    slide.addShape(ctx.shapeRoundRect, {
      x: ctx.width - 1.25 - shadow,
      y: ctx.height - 1.15 - shadow,
      w: 0.72,
      h: 0.72,
      fill: { color: "0A0A0A" },
    ...
    slide.addShape(ctx.shapeRoundRect, {
      x: ctx.width - 1.4 - shadow,
      y: ctx.height - 1.3 - shadow,
      w: 0.72,
      h: 0.72,
      fill: { color: "E5EDD6" },
  ...
  if (theme === "sakura-chroma") {
    // Hard frame + denser accent-tinted offset shadow every slide; chroma strip +
    // stamp orbs stay hero-gated (sakura-chroma-cassette). Cards get accent hard offsets.
    const shadow = 0.14;
    slide.addShape(ctx.shapeRoundRect, {
      x: ctx.width - shadow,
      y: shadow,
    ...
    // Quiet paper tooth on body slides (cassette paper feel without full chroma).
    if (!isHero) {
      for (let i = 0; i < 12; i++) {
        const col = i % 4;
    ...
    if (isHero) {
      ...
      const stripW = (ctx.width - shadow) * 0.52;
      ...
      const stamps = [
        { x: ctx.width - 1.95 - shadow, y: 0.5, color: "E54489", s: 0.48 },
        { x: ctx.width - 1.4 - shadow, y: 0.68, color: "F09131", s: 0.42 },
        { x: ctx.width - 1.65 - shadow, y: 1.15, color: "3F8BC4", s: 0.44 },
        { x: ctx.width - 2.05 - shadow, y: 1.05, color: "3D9F47", s: 0.4 },
        { x: ctx.width - 1.55 - shadow, y: 1.55, color: "F0BC2A", s: 0.36 },
      ];
  ...
  if (theme === "daisy-days") {
    // Hard 3pt frame + charcoal offset shadow + denser pastel dots; daisy/pill
    // stay always-on with charcoal offsets (daisy-days-pastel). Cards get hard offsets.
    const shadow = 0.1;
    slide.addShape(ctx.shapeRoundRect, {
      x: ctx.width - shadow,
      y: shadow,
    ...
    slide.addShape(ctx.shapeRoundRect, {
      ...
      h: ctx.height - 0.2,
      fill: { color: ctx.colors.bg, transparency: 100 },
      line: { color: "2D2D2D", width: 3 },
      rectRadius: 0.28,
    });
    const dots = [
      { x: ctx.width * 0.1, y: ctx.height * 0.14, color: "FDE68A", s: 0.3 },
      { x: ctx.width * 0.86, y: ctx.height * 0.18, color: "F7C8D4", s: 0.4 },
      { x: ctx.width * 0.76, y: ctx.height * 0.78, color: "7ECDC0", s: 0.34 },
      { x: ctx.width * 0.14, y: ctx.height * 0.74, color: "D4A5E8", s: 0.26 },
      { x: ctx.width * 0.48, y: ctx.height * 0.1, color: "A8D8F0", s: 0.22 },
      { x: ctx.width * 0.55, y: ctx.height * 0.86, color: "F7C8D4", s: 0.2 },
    ];
    for (const d of dots) {
      slide.addShape(ctx.shapeOval, {
        x: d.x,
    ...
    // Daisy flower: butter center + white petal stubs + charcoal rim.
    const daisyX = ctx.width - 1.6;
    const daisyY = 0.5;
    const petal = 0.22;
    for (const [px, py] of [
      [0.24, -0.02],
    ...
    slide.addShape(ctx.shapeOval, {
      ...
      h: 0.46,
      fill: { color: "FDE68A" },
      line: { color: "2D2D2D", width: 2.25 },
    });
    // Pill with charcoal hard offset (HTML 3px 3px 0).
    const pillX = 0.55;
    const pillY = ctx.height - 1.0;
    const pillW = 1.05;
    const pillH = 0.38;
    slide.addShape(ctx.shapeRoundRect, {
      x: pillX + 0.04,
  ...
  if (theme === "editorial-tri-tone") {
    // Butter split field + burgundy wash + hard 3pt frame (tri-tone-blocks).
    slide.addShape(ctx.shapeRoundRect, {
      x: ctx.width * 0.55,
      y: 0,
      w: ctx.width * 0.45,
    ...
    // Accent corner blot on heroes (tri-tone energy).
    if (isHero) {
      slide.addShape(ctx.shapeRoundRect, {
        x: ctx.width - 1.45,
        y: 0.55,
  ...
  if (theme === "retro-windows") {
    ...
    // Denser CRT scanlines (HTML 3px pitch).
    for (let i = 0; i < 28; i++) {
      slide.addShape(ctx.shapeRoundRect, {
        x: bevel + inset,
        y: 0.45 + i * 0.24,
        ...
        h: 0.01,
        fill: { color: "000000", transparency: 93 },
        line: { color: "000000", width: 0 },
        rectRadius: 0,
      });
    }
    // Title bar (~22px HTML) + gradient stand-in via dual navy strips.
    slide.addShape(ctx.shapeRoundRect, {
      x: bevel + inset + 0.02,
      y: bevel + inset + 0.02,
    ...
    slide.addShape(ctx.shapeRoundRect, {
      x: bevel + inset + 0.02 + ctx.width * 0.45,
      y: bevel + inset + 0.02,
      w: ctx.width * 0.45 - (bevel + inset + 0.02),
      h: 0.36,
      fill: { color: "0000A0" },
      line: { color: "0000A0", width: 0 },
      rectRadius: 0,
    });
    const winBtns = [
      ctx.width - 1.25 - bevel,
      ctx.width - 0.92 - bevel,
      ctx.width - 0.59 - bevel,
    ];
