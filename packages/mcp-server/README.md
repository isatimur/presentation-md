...
| `scaffold_deck` | Scaffold a schema-native Deck JSON skeleton from a layout recipe (`purpose`: pitch / launch / wrap / paper / …). Pre-wires layouts, asymmetry, dual CTA, notes — agents fill copy, then `audit_deck`. Returns **`studio_share_url`** (`?d=`) by default for one-call editable Studio handoff. Pass `list_purposes: true` for the catalog; `include_share_url: false` to skip the link. |
