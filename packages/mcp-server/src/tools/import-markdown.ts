...
export const importMarkdownTool: ToolDefinition = {
  ...
  description:
    "Convert Marp / md-slides flavored Markdown into presentation-md Deck JSON. Splits on --- slide separators, reads YAML front matter (theme/title/company), and maps headings, lists, tables, quotes, code fences, ```chart, and ```html blocks onto schema layouts.",
  inputSchema: {
    type: "object",
    properties: {
      markdown: {
      ...
      output_path: {
        type: "string",
        description: "Optional path to write the resulting deck JSON",
      },
    },
    required: ["markdown"],
  },
  handler: async (input: Record<string, unknown>) => {
    ...
    return {
      ...
      hint: "Review layout mapping, then audit_deck → render_deck. Use ```chart and ```html fences for chart / custom-html slides.",
    };
  },
};
