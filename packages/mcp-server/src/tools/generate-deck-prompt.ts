...
export const generateDeckPromptTool: ToolDefinition = {
  ...
  handler: async (input: Record<string, unknown>) => {
    ...
    const craftMandate = [
      ...
      "- When theme is unsure: use theme_shortlists_reference (or list_themes browse/include_browse_filters/include_shortlists/mood/query) — offer the same mood chips as the site/Studio gallery, prefer suggested_preview (safe + bold + wildcard), or pick a 3-theme shortlist, call preview_themes (≥2 themes auto-defaults to mode=\"layouts\"; inline PNGs on by default — title + bento + comparison; pass mode=\"title\" only for a cover skim), then lock meta.theme. Studio parity: pick-3 / Generate / Example featured all show Title/Bento/Compare shot strips. Default shortlist for vague briefs: core-defaults (prefer popular:true / browse:\"popular\").",
