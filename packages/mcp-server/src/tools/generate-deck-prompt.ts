...
export const generateDeckPromptTool: ToolDefinition = {
  ...
  handler: async (input: Record<string, unknown>) => {
    ...
    const craftMandate = [
      ...
      "- When theme is unsure: use theme_shortlists_reference (or list_themes browse/include_browse_filters/include_shortlists/mood/query) — offer the same mood chips as the site/Studio gallery, or pick a 3-theme shortlist (safe + bold + wildcard), call preview_themes (≥2 themes auto-defaults to mode=\"layouts\"; pass mode=\"title\" only for a cover skim), then lock meta.theme. Default shortlist for vague briefs: core-defaults (prefer popular:true / browse:\"popular\").",
