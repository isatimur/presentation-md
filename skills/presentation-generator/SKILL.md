...
6. Prefer theme-native PPTX chrome (paper washes, masthead double rules, hero-gated candy/pills/corner blots on `soft-editorial` / `capsule` / `long-table` / `paper-ink`) over inventing flat card stacks — export is a craft surface, not a screenshot dump.
