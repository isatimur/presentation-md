...
| **Discovery** | Studio **mood browse chips** (site parity) + **pick-3 compare** (live auto-on at 3) + Generate **live-by-default** Title/Bento/Compare crops + Example live trio + CLI `--preview-compare` + `preview_themes` **auto-layouts** + `file_url`/`compare_summary` DX | Live style gallery (strong) |
